...
float cameraX, cameraY, cameraZ, treeLocX, treeLocY, treeLocZ,carLocX, carLocY, carLocZ, aspect, mov, inc = 0.001f, angleCamera;
GLuint renderingProgram, vao[numVAOs], vbo[numVBOs], mvLoc, projLoc, obj;
int width, height;
glm::mat4 pMat, vMat, mMat, mvMat, mMatCar;
...
void init(GLFWwindow* window) {
    ...
    //angleCamera = 0.5f;
	treeLocX = -30.0f; treeLocY = -10.0f; treeLocZ = 500.0f;
	carLocX = 0.0f; carLocY = -200.0f; carLocZ = -100.0f;
...
void display(GLFWwindow* window, double currentTime) {
    ...
    mMatCar = glm::translate(glm::mat4(1.0f), glm::vec3(carLocX, carLocY, carLocZ));
    mMatCar = glm::scale(glm::mat4(1.0f), glm::vec3(0.5,0.5 ,0.5));
    mvMat = vMat * mMatCar;
	...
	mMat = glm::translate(mMatCar, glm::vec3(-300.0, 10.0 ,200.0 ));
    mMat = glm::scale(mMat, glm::vec3(400.0, 400.0, 400.0));
    mMat = glm::rotate(mMat, -1.57f, glm::vec3(1.0,0.0,0.0));

    mov += inc;
    if (mov > 1.0) mov = 0.0;

    mMat = glm::translate(mMat, glm::vec3(0.0, mov*8.0, 0.0));// lo rote así que esto traslada en Z
...
int main(void) {
	...
	GLFWwindow* window = glfwCreateWindow(600, 600, "Partial - p4", NULL, NULL);
