...
float cameraX, cameraY, cameraZ, treeLocX, treeLocY, treeLocZ,carLocX, carLocY, carLocZ, aspect;
...
void init(GLFWwindow* window) {
	...
	treeLocX = -170.0f; treeLocY = 0.0f; treeLocZ = 0.0f;
...
void display(GLFWwindow* window, double currentTime) {
    ...
    //mMat = glm::translate(mMat, glm::vec3(0.0, (float)currentTime, 0.0));
    ...
    mMat = glm::translate(glm::mat4(1.0f), glm::vec3(170.0, 0.0, 0.0));
    mMat = glm::scale(mMat, glm::vec3(100.0, 100.0, 100.0));
    mMat = glm::rotate(mMat, -1.57f, glm::vec3(1.0,0.0,0.0));
    mMat = glm::translate(mMat, glm::vec3(0.0, (float)currentTime, 0.0)); // lo rote así que esto traslada en Z
	mvMat = vMat * mMat;
	glUniformMatrix4fv(mvLoc, 1, GL_FALSE, glm::value_ptr(mvMat));
    glUniform1i(obj, 1);
	glBindBuffer(GL_ARRAY_BUFFER, vbo[0]); //Vincula nuestro primer VBO como el búfer activo y almacena los atributos de vértice (coordenadas)
